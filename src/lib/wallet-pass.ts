...
export class WalletPassGenerator {
  static generateQRCode(
    ...
    const qrData = {
      ...
      verifyUrl: `https://verify.iexec-kyc.com/check/${protectedDataAddress.slice(
        -8
      )}`,
    };
  ...
  static async generateAppleWalletPass(
        ...
        backFields: [
          {
            ...
            value:
              "This digital ID was created using iExec's confidential computing technology. Your original documents were never exposed - only age verification and country were computed in a secure enclave.",
          },
          {
            key: "verification",
            label: "Verification Method",
            value:
              "AI-powered document analysis in Trusted Execution Environment (TEE)",
          },
          {
            key: "signature",
    ...
    // TODO: Sign with Apple certificates for production
  ...
  static async generateGoogleWalletPass(
    ...
    // TODO: Sign with Google service account for production
    return JSON.stringify(googlePass, null, 2);
  }

  static generateWalletURLs(
    results: KYCResults,
    protectedDataAddress: string
  ): WalletPass {
    ...
    return {
      ...
      metadata: {
        ...
        expiryDate: new Date(
          results.timestamp + 365 * 24 * 60 * 60 * 1000
        ).toISOString(), // 1 year
      },
    };
  }
